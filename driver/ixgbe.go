...
func (dev *ixgbeDevice) startRxQueue(queueID int) {
	...
	for i := uint16(0); i < queue.numEntries; i++ {
		...
		binary.LittleEndian.PutUint64(rxd.raw[:8], uint64(buf.BufAddrPhy+unsafe.Offsetof(buf.Data)))
...
func (dev *ixgbeDevice) initRx() {
	...
	for i := uint16(0); i < dev.ixy.NumRxQueues; i++ {
		...
		setFlags32(dev.addr, IXGBE_SRRCTL(int(i)), IXGBE_SRRCTL_DROP_EN)
		...
		fmt.Printf("rx ring %v phy addr: %+#v", i, mem.phy)
		hdr := (*reflect.SliceHeader)(unsafe.Pointer(&mem.virt))
		fmt.Printf("rx ring %v virt addr: %+#v", i, hdr.Data)
...
func (dev *ixgbeDevice) initTx() {
	...
	for i := 0; uint16(i) < dev.ixy.NumTxQueues; i++ {
		...
		setReg32(dev.addr, IXGBE_TDBAL(i), uint32(mem.phy&0xFFFFFFFF))
		setReg32(dev.addr, IXGBE_TDBAH(i), uint32(mem.phy>>32))
		setReg32(dev.addr, IXGBE_TDLEN(i), ringSizeBytes)
		...
		txdctl := getReg32(dev.addr, IXGBE_TXDCTL(i))
		...
		setReg32(dev.addr, IXGBE_TXDCTL(i), txdctl)
...
func ixgbeInit(pciAddr string, rxQueues, txQueues uint16) IxyInterface {
	...
	dev.addr = &pciMapResource(pciAddr)[0]
...
func (dev *ixgbeDevice) RxBatch(queueID uint16, bufs []*PktBuf, numBufs uint32) uint32 {
	queue := dev.rxQueues[queueID]
	...
	for ; bufIndex < numBufs; bufIndex++ {
		...
		status := binary.LittleEndian.Uint32(descPtr.raw[8:12]) //bit 64 - 95 of the advanced rx recieve descriptor are the status/error
		if status&IXGBE_RXDADV_STAT_DD != 0 {
			...
			buf.Size = uint32(binary.LittleEndian.Uint16(desc[12:14])) //bit 96 - 111 of the advanced rx recieve descriptor are the length
			...
			newBuf := PktBufAlloc(queue.mempool) //slice has only a single element
			if newBuf == nil {
			...
			//reset descriptor
			//we can't just add the offset of data to the physical address since data is a slice -> extract pointer from slice and translate
			//if the gc frees anything that heÄs not supposed to: runtime.Keepalive
			hdr := (*reflect.SliceHeader)(unsafe.Pointer(&(newBuf[0].Data)))
			binary.LittleEndian.PutUint64(descPtr.raw[:8], uint64(virtToPhys(hdr.Data)))
			binary.LittleEndian.PutUint64(descPtr.raw[8:], uint64(0)) //resets the flags
			queue.virtualAddresses[rxIndex] = newBuf[0]
			bufs[bufIndex] = buf
...
func (dev *ixgbeDevice) TxBatch(queueID uint16, bufs []*PktBuf, numBufs uint32) uint32 {
	...
	for ; sent < numBufs; sent++ {
		...
		//NIC reads from here
		hdr := (*reflect.SliceHeader)(unsafe.Pointer(&(buf.Data)))
		binary.LittleEndian.PutUint64(txd.raw[:8], uint64(virtToPhys(hdr.Data)))
